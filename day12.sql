...
/*동의어(Synonim) 객체
  ...
  사용목적 : 1. 보안측면 계정명(id)와 같은 중요한 정보를 숨기기 위해 별칭을 만듬
            2. 개발 편의성 실제 테이블의 정보가 변경되어도 별칭으로
            사용을 했다면 코드를 수정 안해도됨
*/
-- 시노님 생성 권한 부여(system계정)
GRANT CREATE SYNONYM TO java;
--java 계정에서 동의어 생성
CREATE OR REPLACE SYNONYM mem FOR member;   --default private 시노님 (java계정)
-- java계정에서 study계정에게 mem 을 조회할 수 있는 권한 부여
GRANT SELECT ON mem TO study;   --(java계정)
-- study계정에서 조회-- study계정
SELECT *
FROM java.mem;
-- public synonim 생성
...
NOCACHE;    -- 메모리에 시퀀스 값을 미리 할당할지말지(디폴트 nocache)

SELECT my_seq1.NEXTVAL  -- 조회할때마다 증가함
FROM dual;
...
INSERT INTO tb_click (seq) VALUES ((SELECT MAX(NVL(seq,0)) + 1 FROM tb_click));
SELECT  MAX(NVL(seq,0)) + 1
FROM tb_click;

-- 최소1, 최대 99999999, 1000부터 시작해서 2씩 증가하는
...
SELECT info_seq.NEXTVAL  -- 조회할때마다 증가함
FROM dual;
CREATE TABLE tb_click1(
    seq NUMBER PRIMARY KEY
    ,dt DATE DEFAULT SYSDATE
);
INSERT INTO tb_click1 (seq) VALUES (info_seq.NEXTVAL);