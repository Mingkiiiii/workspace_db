...
/* ºÐ¼®ÇÔ¼ö ·Î¿ì¼Õ½Ç ¾øÀÌ Áý°Ô°ªÀ» »êÃâ ÇÒ ¼ö ÀÖÀ½
...
SELECT ÀÌ¸§, Àü°ø, ÆòÁ¡
     , RANK() OVER(PARTITION BY Àü°ø
                    ORDER BY ÆòÁ¡ DESC) as ¼øÀ§
     , RANK() OVER(
                    ORDER BY ÆòÁ¡ DESC) as ÀüÃ¼¼øÀ§
FROM ÇÐ»ý;
...
-- »óÇ°º° ÃÑÆÇ¸Å±Ý¾×ÇÕ°è¿Í ¼øÀ§¸¦ Ãâ·ÂÇÏ½Ã¿À
-- »óÀ§ 10°³ »óÇ°¸í, ÇÕ°è, ¼øÀ§ Ãâ·Â (cart, prod) Å×ÀÌºí È°¿ë
SELECT *
FROM(SELECT 
    a.prod_id, 
    a.prod_name, 
    SUM(a.prod_sale * b.cart_qty) AS ÇÕ°è,
...
-- NTILE(3) 1 ~ 3 »çÀÌ ¼ö¸¦ º¯È¯ ºÐÇÒÇÏ´Â ¼ö¸¦ ¹öÅ¶ ¼ö¶ó°í ÇÔ
...
SELECT ÀÌ¸§, Àü°ø, ÆòÁ¡
     , LAG(ÀÌ¸§, 1, '1µî') OVER(PARTITION BY Àü°ø
                                        ORDER BY ÆòÁ¡ DESC) as lag
     , LAG(ÆòÁ¡, 1, ÆòÁ¡) OVER(PARTITION BY Àü°ø ORDER BY ÆòÁ¡ DESC)- ÆòÁ¡ as ÆòÁ¡Â÷ÀÌ
FROM ÇÐ»ý
GROUP BY ÀÌ¸§,Àü°ø, ÆòÁ¡;

/* kor_loan_status Å×ÀÌºí¿¡ ÀÖ´Â µ¥ÀÌÅÍ¸¦ ÀÌ¿ëÇÏ¿©
    '¿¬µµº°' 'ÃÖÁ¾¿ù' ±âÁØ °¡Àå ´ëÃâÀÌ ¸¹ÀÌ µµ½Ã¿Í ÀÜ¾×À» ±¸ÇÏ½Ã¿À
    (1) ³âµµº°·Î ÃÖÁ¾¿ùÀÇ µ¥ÀÌÅÍ°¡ ´Ù¸§, 2011Àº 12¿ù,2012³âÀº 12¿ù, 2013Àº 11¿ù ..
        - ¿¬µµº° °¡ÀåÅ« ¿ùÀ» ±¸ÇÔ
    (2) ¿¬µµº° ÃÖÁ¾¿ùÀ» ´ë»óÀ¸·Î ´ëÃâ ÀÜ¾×ÀÌ °¡Àå Å« ±Ý¾×À» ÃßÃâ
