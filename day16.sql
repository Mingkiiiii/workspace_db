...
SELECT department_id, emp_name, hire_date, salary
     ...
     , SUM(salary) OVER(PARTITION BY department_id ORDER BY hire_date
                        ...
                        AND CURRENT ROW) as row1_last
                        -- 1행 이전 + 현재 로우
FROM employees
WHERE department_id IN (30,90);

-- study 계정의 reservation, order_info 테이블을 이용하여 월별 누적금액을 출력하시오
-- RATIO_TO_REPORT : 집합 내 각 항목의 비율을 계산 (집합내에서 해당 값이 차지하는 비용)
SELECT t1.*
